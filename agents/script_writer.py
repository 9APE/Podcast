        ...
        response = self.claude.messages.create(
            ...
            messages=[{
                ...
                "content": (
                    ...
                    f"═══ MRBEAST HOOK FORMULA — first 4 lines decide everything ═══\n\n"
                    f"HOOK LINE 1 — THE CLAIM: Most shocking specific fact. Use a number if possible.\n"
                    f"  WRONG: 'Today we're talking about the World Cup.'\n"
                    f"  RIGHT: '[{host_male.upper()}]: The World Cup bracket has a flaw — and it's been deciding winners since 1986.'\n"
                    f"  RIGHT: '[{host_female.upper()}]: TEMU charged forty million users hidden fees. We found the receipts.'\n\n"
                    f"HOOK LINE 2 — TENSION (4-8 words only): Raise the stakes.\n"
                    f"  'And nobody's covering this.' / 'And it gets worse.' / 'We had to check twice.'\n\n"
                    f"HOOK LINE 3 — THE PROMISE: Exactly what this episode delivers.\n"
                    f"  '[{host_male.upper()}]: We dug into three years of data and found two things nobody's reported.'\n\n"
                    f"HOOK LINE 4 — THE GAP: Unresolved tension that DEMANDS the listener stays.\n"
                    f"  '[{host_female.upper()}]: And what we found changes everything you thought you knew.'\n\n"
                    f"After line 4 the listener is locked in — THEN begin the main conversation.\n\n"
                    f"STRUCTURE:\n"
                    f"HOOK (30s): Lines 1-4 above. Cold open. Zero greetings. No intro.\n"
                    f"SETUP (30s): Frame why this matters TODAY specifically.\n"
