        ...
        response = self.claude.messages.create(
            ...
            messages=[{
                ...
                "content": (
                    ...
                    f"- Reactions must feel unscripted and be SHORT: 1 to 5 words or a punchy question — never a full explanation\n\n"
                    f"CHARACTER REACTIONS (enforce strictly):\n"
                    f"- {host_female} MUST use at least one of these phrases every 3 exchanges: "
                    f"'Hold on.', 'Say that again.', 'That is genuinely shocking.', "
                    f"'Okay but what does that mean for people?', 'Wait — how is that even possible?'\n"
                    f"- {host_male} MUST voice skepticism or push back at least once every 4 exchanges before conceding: "
                    f"'I want to be careful here.', 'That needs context.', 'The number sounds alarming but...', "
                    f"'Let me push back on that.'\n"
                    f"- After {host_male} concedes a point, {host_female} escalates emotionally — never stays neutral\n\n"
                    f"Structure:\n"
